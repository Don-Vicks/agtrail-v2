...
export function AddFarmersModal({ isOpen, onClose }: AddFarmersModalProps) {
  ...
  const lgasForState = useMemo(
    () => sortedLgasForStateCode(inviteForm.stateCode),
    [inviteForm.stateCode],
  )

  const resetInviteForm = () => {
    setInviteForm({ fullName: '', emailOrPhone: '', stateCode: '', lga: '' })
  }
            ...
            <div className="space-y-6">
              <div>
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    type="button"
                    className="flex h-10 items-center gap-2 rounded-md bg-[#1d3d1e] px-4 text-sm font-medium text-white transition-colors hover:bg-black"
                  >
                    <Search className="size-4" />
                    Search
                  </button>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <Select
                      value={inviteForm.stateCode || undefined}
                      onValueChange={(val) => {
                        setInviteForm((prev) => ({
                          ...prev,
                          stateCode: val || '',
                  ...
                  <div>
                    ...
                    <Select
                      value={inviteForm.lga || undefined}
                      onValueChange={(val) =>
                        setInviteForm((prev) => ({ ...prev, lga: val || '' }))
                      }
                      disabled={!inviteForm.stateCode}
                    >
                      <SelectTrigger className="h-10 w-full text-left">
                        <SelectValue
                          placeholder={
                            inviteForm.stateCode ? 'Select LGA' : 'Select state first'
                          }
                        />
                      </SelectTrigger>
                      <SelectContent className="max-h-72">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      const contact = inviteForm.emailOrPhone.trim()
                      if (!fullName || !contact) {
                        toast.error('Please fill in full name and email or phone.')
                        return
                      }
                      const looksEmail = contact.includes('@')
                      if (looksEmail && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(contact)) {
                        toast.error('Enter a valid email or phone number.')
                        return
                      }
                      if (!looksEmail && contact.replace(/\D/g, '').length < 7) {
                        toast.error('Enter a valid email or phone number.')
                        return
                      }
                      const stateName = getStateByCode(inviteForm.stateCode)?.name
                      const loc =
                        stateName && inviteForm.lga.trim()
                          ? `${inviteForm.lga.trim()}, ${stateName}`
                          : stateName || inviteForm.lga.trim() || ''
                      toast.success(
                        loc
                          ? `Invitation sent to ${fullName} (${loc})`
