      ...
      {isLoadingFarms ? (
        ...
        <div className='grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3'>
          {paginatedFarms.map((farm) => {
            const cropsCultivatedCount = apiProducts.filter(
              (p: any) => p.farmId === farm.id,
            ).length
            const certsCount = farm.certifications // Update when certifications model has farm linkages

            return (
              <div
                key={farm.id}
                className='group flex flex-col rounded-md border border-gray-100 bg-white p-6 shadow-sm hover:shadow-md transition-all duration-200'
              >
              ...
              <div className='flex items-start justify-between'>
                <div>
                  ...
                  <p className='mt-0.5 text-xs font-medium text-gray-500'>
                    Upload a new farm certification.
                  </p>
                </div>
                <button
                  type='button'
