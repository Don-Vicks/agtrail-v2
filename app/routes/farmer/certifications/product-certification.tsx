...
export default function ProductCertificationPage() {
  ...
  const { data: productsResp, isLoading: isLoadingProducts } =
  ...
  const uiProducts = useMemo(() => {
    ...
    return apiProducts.map((p: any) => ({
      ...
      location:
        farmsMap.get(p.farmId)?.lga ||
        farmsMap.get(p.farmId)?.region ||
        'Location Not Specified',
      certificationsCount: farmsMap.get(p.farmId)?.certifications.length,
    }))
  ...
  const { mutateAsync: uploadCert, isPending: isSavingCertification } =
  ...
  const handleUpload = async () => {
    if (!selectedProductId || !certType || !uploadedFile) {
      toast.error(
        'Please fill all required fields and upload a certificate file',
      )
      return
    }

    const originalProduct = productsResp?.data?.data?.find(
    ...
    <div className='space-y-6'>
      ...
      <div>
        ...
        <p className='mt-1 text-sm text-gray-500'>
          Upload Certificate for each product
        </p>
      </div>
      ...
      {isLoadingProducts ? (
        ...
        <div className='grid grid-cols-1 gap-6 md:grid-cols-2'>
          {paginatedProducts.map((product) => (
            <div
              key={product.id}
              className='rounded-md border border-gray-100 bg-white p-6 shadow-sm hover:shadow-md transition-shadow'
            >
              {/* Top row: QR + Batch ID */}
              ...
              <div className='mt-4 flex items-center gap-2'>
                <div className='inline-flex items-center gap-1.5 rounded-full border border-green-200 bg-green-50 px-2.5 py-1 text-[11px] font-bold text-green-700'>
                  <CertBadgeIcon />
                  {product.certificationsCount} Certificate
                  {product.certificationsCount !== 1 ? 's' : ''} Uploaded
                </div>
              </div>
          ...
          <div className='relative z-10 w-full max-w-lg overflow-hidden rounded-md bg-white shadow-2xl animate-in fade-in zoom-in-95 duration-200'>
              ...
              <div className='flex items-start justify-between'>
                <div>
                  ...
                  <p className='mt-0.5 text-xs font-medium text-gray-500'>
                    Upload your product quality certificate.
                  </p>
                </div>
                <button
                  type='button'
            ...
            <div className='p-6 space-y-6'>
              ...
              <div className='space-y-2'>
                ...
                <div
                  className={`flex flex-col items-center justify-center rounded-md border-2 border-dashed px-4 py-10 transition-all cursor-pointer ${dragOver
