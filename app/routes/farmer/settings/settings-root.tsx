...
import { useGetUsersProfile, usePutUsersProfile, usePostUsersKyc } from '~/lib/api/generated/users/users'
import { useGetOrganizationsSettings, usePutOrganizationsSettings } from '~/lib/api/generated/organizations-settings/organizations-settings'
...
function AccountSettingsTab() {
  const { user } = useAuth()
  const { data: profileResp, isLoading: isLoadingProfile, refetch: refetchProfile } = useGetUsersProfile()
  const { data: orgResp, isLoading: isLoadingOrg, refetch: refetchOrg } = useGetOrganizationsSettings()

  const { mutate: updateProfile, isPending: isUpdatingProfile } = usePutUsersProfile({
    mutation: {
      onSuccess: () => {
  ...
  const { mutate: updateOrg, isPending: isUpdatingOrg } = usePutOrganizationsSettings({
    mutation: {
      onSuccess: () => {
  ...
  const profile = profileResp?.data?.data as any
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    updateProfile({ data: { name, phoneNumber } })
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl space-y-5">
        ...
        <div className="space-y-1.5 opacity-60">
          ...
          />
          <p className="text-[10px] text-gray-500">Email cannot be changed directly due to security policies.</p>
        </div>
        ...
        <div className="pt-2">
          ...
          >
            {(isUpdatingProfile || isUpdatingOrg) ? 'Saving...' : (
              <>
...
function IdentityVerificationTab() {
  ...
  const appID = import.meta.env.VITE_DOJAH_APP_ID || "5f772c87d30341003e0c8523"
  const publicKey = import.meta.env.VITE_DOJAH_PUBLIC_KEY || "test_pk_OvAQ5aAhwATSKPzOX5vB1Fbv8"
  ...
  const userData = {
    ...
    last_name: user?.name && user.name.split(' ').length > 1 ? user.name.split(' ')[1] : '',
  }
    ...
    <div className="space-y-8">
      ...
      <div className="mx-auto flex max-w-sm flex-col items-center text-center py-6">
        ...
        <p className="mt-1 mb-6 text-sm text-gray-500">
          Click the button below to start your identity verification securely via Dojah.
        </p>
