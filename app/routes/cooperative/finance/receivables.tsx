...
import { cooperativeFarms as farms, farmPerformanceSummary as products, recentReceivables as RECENT_RECEIVABLES } from '~/lib/mock-data/cooperative'
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                ...
                >
                  ...
                  {products.map((p) => (
                    <option key={p.id} value={p.product}>{p.product} ({p.id})</option>
                  ))}
                </select>
                <div className="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                  <svg className="size-4 text-gray-500" viewBox="0 0 20 20" fill="currentColor">
              ...
              {RECENT_RECEIVABLES.map((rec, i) => (
                <tr key={i} className="border-b border-gray-50 last:border-0 hover:bg-gray-50/30">
                  ...
                  <td className="px-4 py-4">
                    <div className="font-bold text-gray-900">{rec.product}</div>
                  </td>
