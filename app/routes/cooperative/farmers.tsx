...
export default function CooperativeFarmers() {
  ...
  const { data: membersResponse, isLoading, error } = useGetOrganizationsMembers()
  const members = membersResponse?.data?.data || []

  const roles = useMemo(() => {
    const rawRoles = members.map(m => m.role || 'Member')
  ...
  const filteredMembers = useMemo(() => {
    return members.filter((member) => {
      const matchesSearch = member.userId.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const farmerStats = useMemo(() => {
    ...
    const verified = members.filter(m => m.role === 'FARMER' || m.role === 'Member').length // Mock logic
    ...
    return (
      <div className="space-y-6">
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-medium text-gray-900 mb-2">Failed to load farmers</h3>
  ...
  return (
    <div className="space-y-6 pb-10 px-1">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 uppercase tracking-tight">Farmers</h1>
          <p className="text-sm text-gray-500 mt-1">Manage and monitor all cooperative members</p>
        </div>
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-bold text-gray-400 uppercase tracking-widest px-1">Role Filter</span>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {filteredMembers.map((member) => (
          <div key={member.id} className="relative rounded-xl border border-gray-200 bg-white p-6 flex flex-col items-center text-center group hover:border-brand/40 hover:shadow-lg transition-all">
            <div className="absolute top-4 right-4 flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
              <Button variant="ghost" size="icon" className="size-8 text-gray-300 hover:text-red-500" onClick={() => setDeleteFarmerId(member.id)}>
            ...
            <div className="relative mb-6">
              <div className="size-24 rounded-2xl overflow-hidden bg-gray-50 border-2 border-white shadow-sm ring-1 ring-gray-100">
                <img 
                  src={`https://ui-avatars.com/api/?name=${encodeURIComponent(member.userId)}&background=random&color=fff&size=128`} 
                  alt={member.userId} 
            ...
            <div className="mb-6">
              <h3 className="text-base font-bold text-gray-900 uppercase tracking-tight truncate w-48 mx-auto" title={member.userId}>{member.userId}</h3>
              <Badge variant="ghost" className="text-[10px] font-bold text-gray-400 uppercase tracking-widest p-0 mt-1">
                {member.role || 'Member'}
              </Badge>
            </div>
            ...
            <div className="w-full space-y-3 mb-6 pt-6 border-t border-gray-50">
              <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-tight text-gray-400">
                <span className="flex items-center gap-2 italic text-gray-300"><Calendar className="size-3" /> Date Joined</span>
                <span className="text-gray-900">{new Date(member.createdAt).toLocaleDateString(undefined, { month: 'short', year: 'numeric' })}</span>
              </div>
              <div className="flex items-center justify-between text-[11px] font-bold uppercase tracking-tight text-gray-400">
                <span className="flex items-center gap-2 italic text-gray-300"><MapPin className="size-3" /> Farms</span>
                <span className="text-brand">3 Linked</span>
              </div>
            </div>
      ...
      <div className="mt-12 border-t border-gray-100 px-6 py-6 flex flex-col sm:flex-row items-center justify-between gap-4 text-[11px] text-gray-400 font-bold uppercase tracking-tight bg-gray-50/20 rounded-xl">
        <div className="flex items-center gap-2">
          ...
          <span className="text-gray-900">{filteredMembers.length} Registered Members</span>
        </div>
