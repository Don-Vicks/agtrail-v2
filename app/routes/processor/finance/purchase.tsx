...
export default function RecordPurchasePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(`Failed to record purchase: ${err.message || 'Unknown error'}`)
    }
  }
    ...
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Acquire and log raw materials from farm produce sources</p>
        </div>
        ...
        <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row sm:items-center justify-between bg-white text-left gap-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 mt-0.5 font-medium">Audit trail of material purchases</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-x-auto">
          {filteredPurchases.length === 0 ? (
            <EmptyState
              ...
              description={search ? `No records found for "${search}"` : "You haven't recorded any processing inputs yet."}
                      ...
                      <td className="px-6 py-5">
                        <div className="flex flex-col">
                          <div className="flex items-center gap-1.5">
                            <span className="font-bold text-gray-900">{purchase.account} Settlement</span>
                            <Badge variant="outline" className="text-[8px] px-1 py-0 bg-blue-50/50 text-blue-600 border-blue-100 font-bold uppercase tracking-wide">Processed</Badge>
                          </div>
                          <span className="text-[10px] text-gray-400 font-medium tracking-wide lowercase">verified by System Agent</span>
                        </div>
                      </td>
