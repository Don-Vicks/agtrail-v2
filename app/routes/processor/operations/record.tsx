...
export default function RecordOperation() {
  ...
  const mappedCycles: UICropCycle[] = useMemo(() => {
    return cropCycles.map((c: any) => {
      ...
      return {
        ...c,
        productName: c.cropName || c.productName || 'Unknown Crop',
    ...
    <div className="space-y-6">
      ...
      {isLoadingCycles ? (
            ...
            <div key={cycle.id} className="group relative rounded-xl border border-gray-200 bg-white p-6 shadow-sm hover:border-brand/40 hover:shadow-lg transition-all flex flex-col">
              ...
              <div className="mb-4 relative z-10">
                ...
                <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mt-0.5 italic">
                  {cycle.variety || 'Elite Seed Stock'}
                </p>
              </div>
        ...
        <div className="flex flex-col items-center justify-center py-16 px-4 text-center bg-white rounded-xl border border-gray-200 border-dashed">
          <div className="bg-brand/5 p-4 rounded-full mb-4 shadow-sm border border-brand/10">
          ...
          <h3 className="text-lg font-bold text-gray-900 uppercase tracking-tight mb-1">No Production Lots Found</h3>
          <p className="text-sm text-gray-500 max-w-sm font-medium">There are no active crop cycles. Start a new production lot to begin recording operations.</p>
        </div>
      )}
